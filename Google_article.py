...
#Далее программа попросит ввести несколько параметров для формирования запроса к google news. Если просто нажимать ввод, по умолчанию запрос будет сделан #за период 30 дней (30d) с ключевым словом Russia, регион поиска United States.

#установка библиотек из requirements
#pip install -r requirements.txt

# установка библиотек из requirements и перенаправляем вывод процесса загрузки в nohup.out в текущую папку
#nohup работает только в Linux
#import os
#path=os.getcwd()
#if not os.path.isfile(path+'/load_lib.txt'):
#    if os.path.isfile(path+'/requirements.txt'):
#        print('В файл load_lib.txt выводится процесс установки модулей.\nНе удаляйте его, чтобы не переустанавливать их при повторном запуске.\nПросмотрите файл на наличие ошибок и при необходимости произведите доустановку модулей вручную.')
 #       os.system('nohup pip install -r requirements.txt > load_lib.txt')
#    else:
#        print('Разместите в общую папку файл requirements.txt и перезапустите программу')
#        import sys
#        sys.exit()
...
date = pd.to_datetime('today').strftime('%Y-%m-%d_%H-%M') #момент получения данных
...
def norm_text(txt_list):
    ...
    stop_words = stopwords.words('english')+['far','is',key.lower(),reg.lower()]
...
df.to_excel('out/googlenews_result'+date+'.xlsx')
...
plt.axis("off")
plt.savefig('out/top'+str(num_words)+'_plasma'+date+'.jpg')
plt.show()
